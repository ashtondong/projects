...
//taking the modulo of sum 1 and 2
    sumT = (sum1 + sum2) % 10;
    return (sumT);
...
// AMEX if number's count == 15 and digits start with 34, 37
// MASTER if number's count == 16 and starts with 51 - 55
// VISA if number's count == 13 or 16 and start with 4

    int modtest;
    modtest = checksum(number);

    int c;
    c = count(number);

    if ((modtest == 0) && (c == 15) && ((fdig == 3 && sdig == 4) || (fdig == 3 && sdig == 7)))
    {
        printf("AMEX\n");
    }
    else if ((modtest == 0) && (c == 16) && (fdig == 5 && 0 < sdig && sdig <= 5))
    {
        printf("MASTERCARD\n");
    }

    else if ((modtest == 0) && (c == 13 || c == 16) && fdig == 4)
    {
        printf("VISA\n");
    }
    else
